...
import com.why168.mapper.DynamicSQLMapper;
...
/*
   ...
   2、where：
       当where标签中有内容时，会自动生成where关键字，并且将内容前多余的[and]或[or]去掉
       当where标签中没有内容时，此时where标签没有任何效果
       注意：where标签不能将其中内容后面多余的[and]或[or]去掉
   3、trim：
       若标签中有内容时：
       prefix|suffix：将trim标签中内容[前面]或[后面][添加]指定内容
       prefixOverrides|suffixOverrides：将trim标签中内容[前面]或[后面][去掉]指定内容
       若标签中没有内容时：trim标签也没有任何效果
   4、choose、when、otherwise，相当于if...else if...else
      when至少要有一个（相当于if...else），otherwise最多只能有一个（相当于else）
   5、foreach
